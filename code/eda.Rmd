...
ggplot(sex_data, aes(x = YEARMONTH, y = EMPCOUNT, color = SEX)) + geom_point() + geom_line() + geom_smooth(method = 'lm') + geom_vline(xintercept = as.Date("2020-03-01")) +  xlab("Date (Year and Month)") + ylab("Standardized Employment Count") + ggtitle("Standardized Employment Count by Year and Month by Sex") + annotate("text", x = as.Date("2020-09-01"), y = 2, label = "COVID_Pandemic", parse = TRUE)
...
ggplot(educ_data, aes(x = YEARMONTH, y = EMPCOUNT, color = EDUC)) + geom_point() + geom_line() + geom_smooth(method = 'lm') + geom_vline(xintercept = as.Date("2020-03-01")) +  xlab("Date (Year and Month)") + ylab("Standardized Employment Count") + ggtitle("Standardized Employment Count by Year and Month by Education Level") + annotate("text", x = as.Date("2020-09-01"), y = 2, label = "COVID_Pandemic", parse = TRUE)
```

```{r}
ggplot(educ_data, aes(x = EDUC, y = EMPCOUNT)) + geom_col()
```
```{r}
age_data = data %>% 
...
ggplot(age_data, aes(x = YEARMONTH, y = EMPCOUNT, color = AGE)) + geom_point() + geom_line() + geom_smooth(method = 'lm') + geom_vline(xintercept = as.Date("2020-03-01")) +  xlab("Date (Year and Month)") + ylab("Standardized Employment Count") + ggtitle("Standardized Employment Count by Year and Month by Age") + annotate("text", x = as.Date("2020-09-01"), y = 2, label = "COVID_Pandemic", parse = TRUE)
```
```{r}
state_data = data %>% 
  group_by(YEAR, MONTH, YEARMONTH, STATEFIP) %>% 
  summarize(EMPCOUNT = median(EMPCOUNTSTD))
ggplot(state_data, aes(x = STATEFIP, y = EMPCOUNT)) + geom_col()
```
